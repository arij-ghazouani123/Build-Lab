...
export default function ProjectsList() {
    ...
    return (
        <table className="table table-hover" style={{ border: "3px solid blue", margin: 30 }}>
            <thead>
                    ...
                    <tr key={item.id} >
                        ...
                        <td>
                            <div style={{ display: 'flex', justifyContent: 'row' }}>
                                ...
                                <button type="button" class="btn btn-danger w-40  rounded-pill" onClick={(event) => {
                                    handleRowClick(item._id); handleDeleteChange(event);
                                }}>Delete</button>
                                <button type="submit" className="btn btn-primary w-40   rounded-pill" onClick={() => handleRowClick(item._id)} ><NavLink className="nav-link" to="/invitation">Invite</NavLink></button>
